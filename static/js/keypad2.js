...
const correctCode2 = '5678'; // Definisci il codice per il keypad 2 (CAMBIALO SE NECESSARIO)

// Funzione per aggiungere cifre all'input del keypad 2
function addKey2(key) {
    if (keypadInput2.length < 4) { // Limite a 4 cifre
        keypadInput2 += key;
...
function checkCode2() {
    if (keypadInput2 === correctCode2) {
        // Controlla se c'è un giocatore in ALFA 2 e se è una coppia (es. VIOLA)
        // Assicurati che currentPlayerAlfa2 sia aggiornato correttamente
        if (currentPlayerAlfa2 && currentPlayerAlfa2.id && currentPlayerAlfa2.id.startsWith('VIOLA')) {
            pressThirdButton2(); // Chiama la funzione per inviare il segnale 'third2'
        } else {
            let message = "Nessuna Coppia 2 (Viola) in ALFA 2.";
            if(currentPlayerAlfa2 && currentPlayerAlfa2.id) {
                message = `Giocatore ${currentPlayerAlfa2.id} in ALFA 2 non è una Coppia 2 (Viola).`;
...
function pressThirdButton2() {
    fetch("/button_press", { // L'endpoint rimane /button_press
        method: "POST",
        headers: {
    ...
    .then(response => {
        if (!response.ok) { // Gestione errori HTTP
             return response.json().then(err => { throw new Error(err.error || `Errore ${response.status}`); });
        }
